...
def active_learning_experiment(run_id, oracle_query_method, task_id):
    ...
    cbis_ddsm_dir = "/usr/xtmp/gk122/mammoproj/data/cbis_ddsm_data/stacks_npy"
    ...
    segmenter = nnunet_model.nnunet_model(base_model_task_id = "Task601_cbis-ddsm")
    ...
    print("what is going on", flush=True)
    segmenter.load_model("/usr/xtmp/vs196/nn_data/nnUNet_preprocessed/Task601_cbis-ddsm")
    #segmenter.initial_lr = 1e-2 #try 1e-3, 5e-2, 1e-1
    print("Loaded in segmenter model!", flush=True)


    #Initialize discriminator
    print(f"RUN_DIR: {run_dir}",flush=True)
    print("\n===INITIALIZING DISCRIMINATOR WITH CBIS-DDSM===", flush=True)
    discriminator = get_init_discriminator(training_dir=cbis_ddsm_dir,
                                           batch_size=BATCH_SIZE_INIT,
                                           num_epochs=INITIAL_DISCRIMINATOR_EPOCHS)
    
    print("DONE WITH DISCRIMINATOR",flush=True)
    # Load initial segmenter and get initial validation accuracy
    print("\n\n\n\n===LOADING INITIAL SEGMENTER (PRETRAINED ON CBIS-DDSM)===", flush=True)
    model_save_path = os.path.join(run_dir, 'all', "Iter"+str(iter_num)+".model")

    #CO- works
    segmenter.save_model(model_save_path)
    print("Saved segmenter",flush=True)
    validation_metric = segmenter.validate(valid_input_dir, valid_output_dir)
    print("finished validation", flush=True)
    print(f"\nMetric of initial segmenter is: {validation_metric}.", flush=True)
    ...
    while True:
        ...
        task_id += 1
        # segmenter_train_dir = os.path.join(os.environ['nnUNet_preprocessed'], f'Task{task_id}_{run_id}')
        # segmenter.trainer.was_initialized=False
        # segmenter.load_new_data(segmenter_train_dir)
        # segmenter.trainer.load_checkpoint(model_save_path)
        # segmenter.update_model(num_epochs=SEGMENTER_UPDATE_EPOCHS_INIT)
        # model_save_path = os.path.join(run_dir, 'all', "Iter" + str(iter_num) +".model")
        # segmenter.save_model(model_save_path)
        segmenter.trainer.epoch += 1
        #increment the epoch here.
        #task_id += 1
