...
def active_learning_experiment(run_id, oracle_query_method, task_id):
    ...
    segmenter.load_model("/usr/xtmp/vs196/nn_data/nnUNet_preprocessed/Task701_cbis-ddsm") # Get base model
    ...
    while True:
        ...
        print("Beginning debugging", flush=True)
        #print(segmenter.trainer.dl_tr.list_of_keys)
        ...
        # print(segmenter.trainer.dl_tr.list_of_keys)
        # print(f"loss of segmenter is: {segmenter.trainer.loss}")
        ...
        segmenter.trainer.loss = DC_and_CE_loss({'batch_dice': segmenter.trainer.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {})
        segmenter.load_new_data(segmenter_train_dir)
       # print("loaded in new data", flush=True)
       # print(segmenter.trainer.dl_tr.list_of_keys, flush=True)
        segmenter.trainer.load_checkpoint(model_save_path)
        #print("finished checkpoint", flush=True)
        #print(segmenter.trainer.dl_tr.list_of_keys, flush=True)
        segmenter.update_model(num_epochs=SEGMENTER_UPDATE_EPOCHS_INIT)
        #print("finished updating model", flush=True)
        #print(segmenter.trainer.dl_tr.list_of_keys, flush=True)
        model_save_path = os.path.join(run_dir, 'all', "Iter" + str(iter_num) +".model")
        segmenter.save_model(model_save_path)
        task_id += 1
